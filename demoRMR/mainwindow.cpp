...
void MainWindow::get_laserdata_and_write_to_map(double robotX,  double robotY,  double robotA, const double setX, const double setY, const int sign)
{
    mutex mux;
    mux.lock();
    mux.unlock();
    ...
    for(int i = 0; i < copyOfLaserData.numberOfScans; i++){
        ...
        if(obstacleDetected){
            double angle = std::abs(std::sin(angle_threshold));

            double d_crit = 0.15/angle;
            if(lidarDist <= d_crit && lidarDist < distance_threshold){
                distance_check = true;
            }
            else
        ...
        if(distance_check && obstacleDetected){
            double temp1 = lidarDist*std::sin(lidarAngle + robotA);
            double temp2 = lidarDist*std::cos(lidarAngle + robotA);
            obstacleCoord.x = robotX + temp1;
            obstacleCoord.y = robotY + sign*temp2;
            std::cout << "Found obstacle at: xp = " << obstacleCoord.x << ", yp =" << obstacleCoord.y << std::endl;
            std::cout << "tempX = " << temp1 << ", tempY =" <<temp2<< std::endl;
    ...
    for(int i = obstacleLidarIndex; ;i--){
        ...
        if(leftCornerDetected){
            ...
            obstacleCornerLeft.y = robotY + sign*oldLidarDistLeft*std::sin(oldLidarAngleLeft + robotA);
    ...
    for(int i = obstacleLidarIndex; ; i++){
        ...
        if(rightCornerDetected){
            ...
            obstacleCornerRight.y = robotY + sign*oldLidarDistRight*std::sin(oldLidarAngleRight + robotA);
    ...
    if(leftCornerDetected){
        ...
        ypLeft = robotY + sign*temp*std::sin(calculatedAngle + robotA);

        if(checkAccessibility(xpLeft, ypLeft, robotX, robotY, sign)){
    ...
    if(rightCornerDetected){
        ...
        ypRight = robotY + sign*temp*std::sin(calculatedAngle + robotA);

        if(checkAccessibility(xpRight, ypRight, robotX, robotY,  sign)){
...
int MainWindow::processThisLidar(LaserMeasurement laserData)
{
    ...
    if(controlType == 0 && mission_started){
        ...
        if(tempSetPoint.x != -1){
            f = std::bind(&MainWindow::get_laserdata_and_write_to_map,this,robotX, robotY, robotAngle, tempSetPoint.x,tempSetPoint.y,1);
            std::async(std::launch::async, f);
        }
        else if(!set_point.xn.empty()){
            f = std::bind(&MainWindow::get_laserdata_and_write_to_map,this,robotX, robotY, robotAngle, set_point.xn[set_point.xn.size()-1],set_point.yn[set_point.yn.size()-1],1);
            std::async(std::launch::async, f);
        }
    }
    else if(controlType == 1 && mission_started){

       double robotX2 = robotCoordMap.x;
       double  robotY2 = robotCoordMap.y;
        double robotAngle2 = robotCoordMap.a*TO_RADIANS;
        ...
        if(tempSetPoint.x != -1){
            f = std::bind(&MainWindow::get_laserdata_and_write_to_map,this,robotX2, robotY2, robotAngle2, tempSetPoint.x,tempSetPoint.y,-1);
            std::async(std::launch::async, f);
        }
        else if(!set_point_map.xn.empty()){
            f = std::bind(&MainWindow::get_laserdata_and_write_to_map,this,robotX2, robotY2, robotAngle2, set_point_map.xn[set_point_map.xn.size()-1],set_point_map.yn[set_point_map.yn.size()-1],-1);
            std::async(std::launch::async, f);
        }
    }
...
double MainWindow::calculateDistance(const double xp, const double yp, const double setX, const double setY, const double robotX, const double robotY){
    double ex = xp - robotX;
    double ey = (yp - robotY);
...
bool MainWindow::checkAccessibility(const double xp, const double yp, const double robotX, const double robotY, const int sign)
{
    std::cout << "xp check= " << xp << ", yp check= " << yp << std::endl;
    double search_offset = 0.155;
    mutex m;
    m.lock();
    for(int k=0;k<copyOfLaserData.numberOfScans/*360*/;k++){
        ...
        double xp2 = robotX + lidarDist*std::cos(lidarAngle);
        double yp2 = robotY + sign*lidarDist*std::sin(lidarAngle);
    ...
    else
    {
        if(updateLaserPicture==1)
        {
            ...
            if(controlType == 0 && !set_point.xn.empty()){
                ...
                for(int i = 0; i < set_point_map.xn.size(); i++){
                    double ex = (set_point_map.xn[i] - robotCoordMap.x);
                    double ey = -(set_point_map.yn[i] - robotCoordMap.y);
                    double eDist = sqrt(ex*ex + ey*ey)/20.0*1000.0;

                    double eRot = std::atan2(ey,ex) - robotCoordMap.a*TO_RADIANS;
                    eRot = std::atan2(std::sin(eRot), std::cos(eRot));
                    if(eRot < 0){
                        eRot  = eRot +2*PI;
                    }
            ...
            if(controlType == 0 && tempSetPoint.x != -1){
                pero2.setStyle(Qt::SolidLine);
                pero2.setWidth(3);
            ...
            else if(controlType == 1 && tempSetPoint.x != -1){
                pero2.setStyle(Qt::SolidLine);
                pero2.setWidth(3);
                pero2.setColor(Qt::cyan);
                painter.setBrush(Qt::cyan);
                ...
                double ex = (tempSetPoint.x - robotCoordMap.x)*1000;
                double ey = -(tempSetPoint.y - robotCoordMap.y)*1000;
                double eDist = sqrt(ex*ex + ey*ey)/20;
            ...
            mux.unlock();
            mux.lock();
            if(controlType == 0 && obstacleCoord.x != -1 && obstacleCoord.y != -1){
                pero2.setStyle(Qt::SolidLine);
                pero2.setWidth(3);
            ...
            else if(controlType == 1 && obstacleCoord.x != -1 && obstacleCoord.y != -1){
                pero2.setStyle(Qt::SolidLine);
                pero2.setWidth(3);
                pero2.setColor(Qt::red);
                painter.setBrush(Qt::red);
                painter.setPen(pero2);
                double ex = (obstacleCoord.x - robotCoordMap.x)*1000;
                double ey = -(obstacleCoord.y - robotCoordMap.y)*1000;
                double eDist = sqrt(ex*ex + ey*ey)/20;
            ...
            if(controlType == 0 && obstacleCornerRight.x != -1 && obstacleCornerRight.y != -1){
                pero2.setStyle(Qt::SolidLine);
                pero2.setWidth(3);
            ...
            else if(controlType == 1 && obstacleCornerRight.x != -1 && obstacleCornerRight.y != -1){
                pero2.setStyle(Qt::SolidLine);
                pero2.setWidth(3);
                pero2.setColor(Qt::darkCyan);
                painter.setBrush(Qt::darkCyan);
                painter.setPen(pero2);
                double ex = (obstacleCornerRight.x - robotCoordMap.x)*1000;
                double ey = -(obstacleCornerRight.y - robotCoordMap.y)*1000;
                double eDist = sqrt(ex*ex + ey*ey)/20;
            ...
            if(controlType == 0 && obstacleCornerLeft.x != -1 && obstacleCornerLeft.y != -1){
                pero2.setStyle(Qt::SolidLine);
                pero2.setWidth(3);
            ...
            else if(controlType == 1 && obstacleCornerLeft.x != -1 && obstacleCornerLeft.y != -1){
                pero2.setStyle(Qt::SolidLine);
                pero2.setWidth(3);
                pero2.setColor(Qt::darkGray);
                painter.setBrush(Qt::darkGray);
                painter.setPen(pero2);
                double ex = (obstacleCornerLeft.x - robotCoordMap.x)*1000;
                double ey = -(obstacleCornerLeft.y - robotCoordMap.y)*1000;
                double eDist = sqrt(ex*ex + ey*ey)/20;
...
\
    if(mission_started){
        if(controlType == 1 && tempSetPoint.x != -1 && tempSetPoint.y != -1){
            robot_odometry.robot_odometry(robotdata, true, robotCoordMap, -1);
            robot_motion_reg.robot_movement_reg(tempSetPoint.x, tempSetPoint.y, robotCoordMap, robot_motion_param,-1);
        }
        else if(controlType == 0 && tempSetPoint.x != -1 && tempSetPoint.y != -1){
        ...
        if(forwardspeed == 0.0 && rotationspeed != 0.0){
            ...
            if(controlType == 0 && !set_point.xn.empty()){
                if((robotCoord.x >= set_point.xn[set_point.xn.size()-1]-0.2) && (robotCoord.x <= set_point.xn[set_point.xn.size()-1]+0.2) &&
                    (robotCoord.y >= set_point.yn[set_point.yn.size()-1]-0.2) && (robotCoord.y <= set_point.yn[set_point.yn.size()-1]+0.2)){
                    set_point.xn.pop_back();
                    set_point.yn.pop_back();
                }
            }
            else if(controlType == 1 &&!set_point_map.xn.empty()){
                if((robotCoordMap.x >= set_point_map.xn[set_point_map.xn.size()-1]-0.2) && (robotCoordMap.x <= set_point_map.xn[set_point_map.xn.size()-1]+0.2) &&
                    (robotCoordMap.y >= set_point_map.yn[set_point_map.yn.size()-1]-0.2) && (robotCoordMap.y <= set_point_map.yn[set_point_map.yn.size()-1]+0.2)){
                    set_point_map.xn.pop_back();
                    set_point_map.yn.pop_back();
                }
            }
            if(controlType == 0 && tempSetPoint.x != -1){
                if((robotCoord.x >= tempSetPoint.x-0.2) && (robotCoord.x <= tempSetPoint.x+0.2) &&
                    (robotCoord.y >= tempSetPoint.y-0.2) && (robotCoord.y <= tempSetPoint.y+0.2)){
                    tempSetPoint.x = -1;
                    tempSetPoint.y = -1;
                }
            }
                ...
                if((robotCoordMap.x >= tempSetPoint.x-0.2) && (robotCoordMap.x <= tempSetPoint.x+0.2) &&
                    (robotCoordMap.y >= tempSetPoint.y-0.2) && (robotCoordMap.y <= tempSetPoint.y+0.2)){
                    tempSetPoint.x = -1;
                    tempSetPoint.y = -1;
                }
            }
